        ...
        for file in glob.glob(file_path + "\\*"):  # batch wise logs folder
            ...
            for sub_file in glob.glob(file + "\\5208693-5192-N.log"):  # individual candidate log file
                ...
                try:
                    ...
                    # test_dict = dict([(key, [val1, val2, val3,]) for key, val1, val2, val3 in zip(dataframe['QuestionID'], dataframe['Timestamp'],dataframe['Section Name'],dataframe['QuestionID'])], orient='list')
                    # test_dict = dataframe.set_index('QuestionID').T.to_dict('series')
                    # print('hi')
                    ...
                    for index, item in enumerate(input_csv_data):
                        if item[4] != -1 and item[9] == 'RS':
                            ...
                            if unique_key not in unique_questions:
                                unique_questions[unique_key] = list()
                            unique_questions[unique_key].extend([output_row])
                    ...
                    for index, item in enumerate(sub_file_output_rows):  # creating dictionary with question and number of attempts
                        ...
                        for x in range(len(sub_file_output_rows[index])):
                            ...
                            if len((sub_file_output_rows[index])) > 1:
                                if x == 0:
                                    sub_file_output_rows[index][x].append("First-attmpt")
                                elif x == len(sub_file_output_rows[index])-1:
                                    sub_file_output_rows[index][x].append("Final-attmpt")
                                else:
                                    sub_file_output_rows[index][x].append("-")

                            else:
                                sub_file_output_rows[index][x].append("Single-attmpt")
                            sub_file_output_rows_final.insert(index, sub_file_output_rows[index][x])
                    ques_attmpt_list = list(ques_attmpt.values())
                    if ques_attmpt_list:
                        avg_attmpt = round(mean(ques_attmpt_list), 2)
                    else:
                        avg_attmpt = 0
