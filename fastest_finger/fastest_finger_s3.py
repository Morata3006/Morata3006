...
def process_file(file_path, question_dict, master_dict, registration_dict, col_headers, count_list):
    try:
        ...
        client_id = file_path.split("/")[0].split('_')[0]
        exam_name = file_path.split("/")[0].split('_')[1]
        log_message(
            "Current working directory: " + file_path,
            "info",
            "system_log",
        )
        rack_name = file_path.split('/')[-2]
        Value = 1

        batch_objs = bucket.meta.client.list_objects(Bucket=bucket.name, Delimiter='/', Prefix=file_path).get('CommonPrefixes')
        for batch in batch_objs:
            file_name = batch['Prefix'].split("/")[-2]
            read_files = []
            output_rows = []

            log_obj = bucket.objects.filter(Prefix=batch['Prefix'])
            for log in log_obj:
                log_body = log.get()['Body'].read()

                try:
                    unique_questions = {}
                    exam_id = log.key.split("/")[-1].split("-")[0]  # SO711311*** (11 alphanumeric)
                    eed_id = log.key.split("/")[-1].split("-")[1]  # 218132 (6 digit)
                    dataframe = (
                        pd.read_csv(
                            io.BytesIO(log_body),
                            delimiter="|",
            ...
            read_files.append([batch['Prefix']])
            with open(
...
def process_eod_file(file_path, question_dict, master_dict, registration_dict, col_headers, cal, count_list):
    try:
        ...
        for file in glob.glob(file_path + "\\*"):  # batch wise logs folder
            ...
            for sub_file in glob.glob(file + "\\510527066-101669-N.log"):  # individual candidate log file
                ...
                try:
                    ...
                    if not output_row and item:  # for not attempted single row addition
                        if item[4] == -1:
                            ...
                            if key_master in master_dict.keys():
                                # CRR lookup operation
                                date_split = str(master_dict[key_master][6]).split('-')
                                month_name = cal[date_split[1]]
                                exam_date = date_split[-1].split(' ')[0] + month_name
                                key_ques = str(output_row[0]) + "_" + str(output_row[2]) + "_" + str(
                                    int(output_row[6])) + "_" + exam_date  # diamond_examid_QuestionID_date
                                if key_ques in question_dict.keys():
                                    output_row.append(question_dict[key_ques][3])  # Medium Code
                                    output_row.append("NA")  # batch time
...
def crr_dict(base, c_name, e_name, mapping_dict_crr):
    question_set = bucket.objects.filter(Prefix=base + "fastestfinger/CRR/")
    ...
    for obj in question_set:
        ...
        str_body = obj_body.decode()
        if '","' in str_body:
            question_partial = pd.read_csv(io.BytesIO(obj_body), delimiter=',').reset_index(drop=True)
        elif '";"' in str_body:
            question_partial = pd.read_csv(io.BytesIO(obj_body), delimiter=';').reset_index(drop=True)
        e_date = obj.key.split('/')[-1].split('.')[0].split('_')[-1]
...
def main(list_of_folders):  # list of clients_exams
    ...
    for folder in list_of_folders:
        list_of_files = []
        rack_objs = bucket.meta.client.list_objects(Bucket=bucket.name, Delimiter='/', Prefix=folder + 'fastestfinger/raw/').get('CommonPrefixes')
        for racks in rack_objs:
            list_of_files.append(racks['Prefix'])
        client_name = folder.split('_')[0]
        exam_name = folder.split('_')[-1].replace('/', '')
        files = [file for file in list_of_files if "Master" not in file and "Exception" not in file]
        for i in list_of_files:
            exam_rack_name = i.split('/')[-2]
            create_output_files(client_name, exam_rack_name, exam_name)

        calendar_dict = {str(index).zfill(2): month.upper() for index, month in enumerate(calendar.month_abbr) if month}

        mapping_dict_crr = {}
        map_crr_objs = bucket.objects.filter(Prefix=folder + "fastestfinger/mapping_file/")
        for obj in map_crr_objs:
            file_body = obj.get()['Body'].read()
            df = pd.read_csv(io.BytesIO(file_body), header=0, delimiter=",", low_memory=False)
            df_list = df.values.tolist()
            for line in df_list:
                mapping_dict_crr[line[0]] = line[1]

        mapping_dict_reg = {}
        map_reg_objs = bucket.objects.filter(Prefix=folder + "registration/mapping_files/")
        for obj in map_reg_objs:
            file_body = obj.get()['Body'].read()
            df_reg = pd.read_csv(io.BytesIO(file_body), header=0, delimiter=",", low_memory=False, encoding='unicode_escape')
            df_list_reg = df_reg.values.tolist()
            for line in df_list_reg:
                if line[2] == exam_name and line[5] == client_name and line[3] == 'yes':
                    mapping_dict_reg[line[0]] = line[1]

        master_set = bucket.objects.filter(Prefix=folder + "fastestfinger/Master/")
        registration_set = bucket.objects.filter(Prefix=folder + "registration/raw/")
        ...
        #  master data lookup
        final_master_dict = {}
        for obj in master_set:
            file_content = obj.get()['Body'].read()
            master_partial = pd.read_excel(io.BytesIO(file_content), usecols=default_cols). reset_index(drop=True)
        ...
        reg_headers = []
        for obj in registration_set:
            obj_body = obj.get()['Body'].read()
            registration_partial = pd.read_excel(io.BytesIO(obj_body),).reset_index(drop=True)
            registration_dataframe = registration_dataframe.append(registration_partial)
        if not registration_dataframe.empty:
        ...
        # CRR lookup
        crr_dataframe, crr_data_list, crr_headers = crr_dict(folder, client_name, exam_name, mapping_dict_crr)
        final_question_dict = {}
        count_list = []
        final_count_list = []
        if len(crr_headers) > 10:
            ...
            for x in files:
                p = process_file(x, final_question_dict, final_master_dict, final_registration_dict, reg_headers,
                                 count_list)
        else:
            examid_idx = crr_dataframe.columns.get_loc('exam_id')
            qst_idx = crr_dataframe.columns.get_loc('question_no')
            ...
            # print(flat_list.count(1))
            for x in files:
                p = process_eod_file(x, final_question_dict, final_master_dict, final_registration_dict, reg_headers, calendar_dict,
                                 count_list)
...
# folders = glob.glob(base_dir + '\\IIBF_exam')
folders = []
