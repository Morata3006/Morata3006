            ...
            if file not in Master_list:
                ...
                for sub_file in glob.glob(file + "/*"):  # individual candidate logs
                    if os.path.getsize(sub_file) > 0:
                        try:
                            sub_file_name = sub_file.split('\\')[-1]
                            unique_questions = {}
                ...
                with open(
                        output_dir + client_id + "_fastest_finger/" + rack_name + "/" + file_name + "/" + sub_file_name + ".csv",
                        "w",
                        newline="",
                ) as f:
                    writer = csv.writer(f)
...
def create_output_files(client_value, rack_value, batch_val, file_name):
    if not os.path.exists(output_dir + client_value + "_fastest_finger/" + rack_value + batch_val):
        os.makedirs(output_dir + client_value + "_fastest_finger/" + rack_value + "/" + batch_val)
        open(output_dir + client_value + "_fastest_finger/" + rack_value + "/" + batch_val + file_name + ".csv", "w", newline="")
...
if __name__ == '__main__':
    ...
    temp_files = glob.glob(base_dir + input_dir + "*/*/*")
    for i in temp_files:
        diamond_file = i.split('\\')[-1]
        diamond_rack_name = i.split('\\')[-3]
        diamond_batch = i.split('\\')[-2]
        create_output_files(diamond_client, diamond_rack_name, diamond_batch, diamond_file)
