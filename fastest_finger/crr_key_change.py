...
def process_file(file_path, question_dict, master_dict, cal, count_list):
    try:
                ...
                try:
                    ...
                    for index, item in enumerate(input_csv_data):
                        ...
                        if key_master in master_dict.keys():
                            ...
                            key_ques = str(output_row[0]) + "_" + str(output_row[2]) + "_" + str(int(output_row[6])) + "_" + exam_date  # diamond_examid_QuestionID_date
...
def main(list_of_folders):
    ...
    for folder in list_of_folders:
        ...
        question_dataframe = pd.DataFrame(
            columns=["Exam Code", "Membership No.", "Medium Code", "Question Paper Id", "Question Id", "Candidate Response", "Correct Answer"])
        ...
        for question_template in question_set:
            ...
            question_partial = pd.read_csv(
                ...
                delimiter=",",
                usecols=[
                    "Exam Code","Membership No.", "Medium Code","Question Paper Id","Question Id","Candidate Response","Correct Answer"
                ],
                low_memory=False,
            ).reset_index(drop=True)
            ...
            question_temp = [final_question_dict.update({client_name + "_" + str(item[1]) + "_" + str(item[-3]) + "_" + e_date: item}) for
                             item in question_csv_data]  # diamond_examid_qstno_date
        ...
        for x in files:
            p = process_file(x, final_question_dict, final_master_dict, calendar_dict, count_list)
            # print(p)
    log_message(
